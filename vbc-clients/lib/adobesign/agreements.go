...
func (s *AgreementService) ListAgreements(ctx context.Context) (interface{}, error) {

	req, err := s.client.NewRequest("GET", agreementsPath+"?pageSize=5", nil)
	if err != nil {
		return nil, err
	}
...
func (s *AgreementService) FormFields(ctx context.Context, agreementId string) (interface{}, error) {
	u := fmt.Sprintf("%s/%s/formFields", agreementsPath, agreementId)
